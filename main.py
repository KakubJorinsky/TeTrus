...
def draw_high_score(surface):
    """ Zobrazení nejfiššího skóre v hlavním menu """
    font = get_font(50)
    text = font.render(f"Nejfišší skóre: {high_score}", True, (0, 0, 0))
...
def get_scaled_bg3():
    width, height = SCREEN.get_size()
    return pygame.transform.scale(pygame.image.load("assets/Background3.png"), (width, height))

def get_scaled_bg4():
    width, height = SCREEN.get_size()
    return pygame.transform.scale(pygame.image.load("assets/Background4.png"), (width, height))

def get_scaled_bg5():
    width, height = SCREEN.get_size()
    return pygame.transform.scale(pygame.image.load("assets/Background5.png"), (width, height))
...
def play():
    ...
    shape_pos = [3, 0]
    play_music("assets/yorck_march.ogg", 0.25)
    while True:
        screen_width, screen_height = SCREEN.get_size()
        center_x = screen_width // 2
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if OPTIONS_BACK.checkForInput(OPTIONS_MOUSE_POS):
                    update_high_score()
                    play_music("assets/prus_march.ogg", 0.1)
                    return
        ...
        SCREEN.fill((0, 0, 0))
        SCREEN.blit(get_scaled_bg5(), (0, 0))
        
        OPTIONS_BACK = Button(image=None, pos=(100, 100),
                              text_input="Zurück", font=get_font(75),
...
def options():
    ...
    while True:
        OPTIONS_MOUSE_POS = pygame.mouse.get_pos()
        SCREEN.fill("white")
        SCREEN.blit(get_scaled_bg4(), (0, 0))
        ...
        options_list = [
            {"label": "Kadett", "pos": (center_x, 240)},
            {"label": "Unteroffizier", "pos": (center_x, 340)},
            {"label": "Feldmarschall", "pos": (center_x, 440)}
        ]
...
def main_menu():
    ...
    while True:
        ...
        MENU_TEXT = get_font(100).render("TeTrus", True, "black")
        MENU_TEXT2 = get_font(50).render("Kampf gegen die Blöcke!", True, "#47340e")
        screen_width, screen_height = SCREEN.get_size()
        center_x = screen_width // 2
        MENU_RECT = MENU_TEXT.get_rect(center=(center_x, 100))
        MENU_RECT2 = MENU_TEXT2.get_rect(center=(center_x, 160))

        PLAY_BUTTON = Button(image=pygame.image.load("assets/Play Rect.png"), pos=(center_x, 250), 
                            text_input="Hrát", font=get_font(75), base_color="black", hovering_color="red", base_color2=None)
        OPTIONS_BUTTON = Button(image=pygame.image.load("assets/Options Rect.png"), pos=(center_x, 400), 
                            text_input="Obtížnost", font=get_font(75), base_color="black", hovering_color="red", base_color2=None)
        QUIT_BUTTON = Button(image=pygame.image.load("assets/Quit Rect.png"), pos=(center_x, 550), 
                            text_input="Zavřít", font=get_font(75), base_color="black", hovering_color="red", base_color2=None)
